...
const API_BASE =
  process.env.API_BASE_URL || process.env.NEXT_PUBLIC_API_BASE || "http://localhost:8000";
...
export default async function PuzzleGamePage({ params }: { params: { slug: string } }) {
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-neutral-300">
              Part of GuessSenpai Daily{formattedDate ? ` — ${formattedDate}` : ""}
            </p>
          </div>
          <Link
            href="/games/daily"
